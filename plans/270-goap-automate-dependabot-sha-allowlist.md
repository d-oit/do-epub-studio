...
| 3 | Dogfood and record: observe one live bump; update this plan and ADR-247 status | Phase 2 observed end-to-end on a real Dependabot PR; learnings captured via the `learn` skill | IN PROGRESS (live no-op path OBSERVED 2026-09-15 09:15–09:17 UTC: the scheduled Tuesday npm run created 5 real dependabot[bot] PRs (#1132–#1136) and the Dependabot Auto-merge workflow executed the new automation on each — checkout base, `Verify and allowlist bumped action SHAs` step ran `scripts/allowlist-dependabot-shas.sh` against the real PR diff, `✓ nothing to allowlist` + `No new SHAs to allowlist.` (clean no-op on non-actions ecosystems, npm bumps unaffected), approve step correctly withheld while checks were pending per GOAP-224. Remaining for Phase 3: observe the SHA-append path on a real github-actions bump — chromaui/action bait (v18.7.2 on main via #1128) awaits the scheduled weekly run Mon 2026-09-22 09:00 UTC; fallback if no re-bump PR by end of that day: restore v18.8.1 manually and close Phase 3 as verified-by-tests) |
