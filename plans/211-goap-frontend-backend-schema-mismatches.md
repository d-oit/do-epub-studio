...
**Date:** 2026-08-02
**Status:** ✅ COMPLETED
**PR:** [#896](https://github.com/d-oit/do-epub-studio/pull/896)
...
- [x] `ProgressUpdateSchema` accepts partial locator (cfi-only for progress saves)
- [x] Offline progress sync uses PUT method matching worker route
- [x] Offline highlight sync sends nested `locator` object matching `HighlightCreateSchema`
- [x] Offline bookmark sync sends `chapterRef` (not `chapter`) and non-empty `selectedText`
- [x] Offline comment sync sends nested `locator` object
- [x] Offline resolve/reply items have `type` field and are handled correctly in `syncItem`
- [x] Dead `if (!navigator.onLine)` branches after successful API calls are removed
- [x] Progress `parseLocatorRow` uses `'progress'` entity type
- [x] Legacy `AnnotationLocatorSchema` removed from schema package
- [x] `sync.test.ts` validates payloads against real Zod schemas
- [x] `./scripts/quality_gate.sh` passes
- [x] PR created and CI green
