...
| Security   | high     | FORBID_TAGS-only replaced with explicit ALLOWED_TAGS allowlist (EPUB_BODY__, EPUB_HEAD__, EPUB_STRUCTURAL__, SAFE_SVG__). Denylist-only was a known foot-gun: a future DOMPurify version shipping a new default-permitted tag would let it through silently. The new allowlist also drops form/input/button/select/textarea. |
