...
- **F9 (P2, executed 2026-09-10, branch `feat/complete-executable-plan-backlog`)** Partial ✅/⛔: **@vitejs/plugin-react 6.1.0 → 6.1.1** landed lockfile-only (specifier `^6` unchanged; integrity `sha512-yxLaQV9gkhS8ezJqCM6+ndU7mDY6gqAg75NQ+0IjwEI8IYOmQCgkRwHKVSfWXW076DsqMo0Dk+0FK1U+M5RgFw==`; release 2026-08-28: `compiler.logDiagnostics` option + env-sourcemap respect under `builder.sharedPlugins` #1439; deps/peers/engines identical). **@intity/epub-js 0.3.97/0.3.98 blocked, 0.3.96 retained**: both publish `main: lib/index.js` + `module: src/index.js` but ship neither (only `dist/` + `types/`), so the runtime import in `packages/reader-core/src/epub-loader.ts:1` fails `MODULE_NOT_FOUND`; 0.3.98 also repackages deps (+jsdom, +resize-observer, marked 15→18, lodash 4.17→4.18, −core-js). No GitHub release bodies (0.3.97 bare tag, 0.3.98 no page). Remainder already ≥ target (vitest/coverage 4.1.11, hono 4.13.7, wrangler 4.125.0, workers-types 5.20260822.1, vite 8.2.2, turbo 2.10.11, dompurify 3.4.14, storybook 10.5.10, js-yaml 5.4.1, impeccable 4.0.1, sentry 10.73/10.74, typescript-eslint 8.67.0). No `package.json` modified; `pnpm install --frozen-lockfile` passes; `pnpm audit --audit-level=high` green.
