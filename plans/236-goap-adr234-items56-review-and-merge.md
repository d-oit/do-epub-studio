...
- **Recovery-code single-use race (WEAKN-002):** `verifyRecoveryCode` is now a single atomic SQLite JSON1 UPDATE (`json_each`/`json_group_array` + `EXISTS`), so concurrent redemptions cannot clobber each other.
