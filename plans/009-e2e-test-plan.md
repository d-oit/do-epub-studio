...
## Execution Reality Check (2026-04-15)

The catalog below remains the long-term target matrix. Current implementation has strong reader/admin coverage, but several tracks are still planned (see `plans/010-optimization-quality-backlog.md`).

### Active gaps

- Component-level tests for annotation/admin pages need deeper regression coverage.
- PR-time E2E should run a smoke-tag subset; full matrix should remain nightly.
- Performance/security scenarios are partially specified but not yet fully automated in CI.

### Playwright baseline policy

- Chromium + Firefox run by default for local and CI reliability.
- WebKit runs only when `PLAYWRIGHT_INCLUDE_WEBKIT=1` is set, preventing known Linux dependency false negatives.
- Failed tests retain trace/video/screenshot artifacts to accelerate diagnosis.
