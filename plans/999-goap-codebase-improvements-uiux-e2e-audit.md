...
Known symptom left open (not attributable to the fixes above): the reader occasionally mounts to an empty root immediately after a client-side login navigation and recovers on reload. It was observed three times during this verification without a captured console or page error; every attempt to reproduce it deliberately (demo login, credentials login, reload) rendered correctly. Recorded here rather than closed.
