...
Symptom investigated and closed (2026-09-20, follow-up round). Two separate things were tangled together:

1. **A real defect, fixed here:** the API layer classified aborts backwards. `AbortController.abort(new DOMException('Request timeout'))` produces a DOMException whose `name` is `Error`, while a caller cancellation produces `AbortError` — so the branch that logged `api.timeout` fired only for *cancellations* (React effect cleanup), and a genuine expired deadline fell through to `api.network-error`. The retry guard `error instanceof TypeError && error.message !== 'Request timeout'` was unreachable. `useReaderEpub` additionally logged cleanup-driven loader teardown as `reader.epub_init_failed` at error level. Both are fixed (`api.cancelled` at info for cancellations, `TimeoutError` named deadline → `api.timeout`, never retried; `reader.epub_init_aborted` for superseded inits) with tests that fail on the pre-fix code.
2. **The blank/stall observations were harness-side.** Instrumented runs (hooks surviving client-side navigation) rendered the reader with `root: 2, iframes: 1` and chapter text, and logged no page error; when a probe hung, the *entire* managed Chromium was gone (0 `chrome-headless-shell` processes) on an idle machine with 8 GB free and no OOM record, and no renderer was ever CPU-bound. That matches the managed browser's idle-tab lifecycle across tool-call boundaries, and the CI reader lanes (`demo-login.spec.ts`, `login-and-book-load.spec.ts`) plus repeated fresh-browser runs contradict a product bug. The mislabeled telemetry above is what made it read as a timeout or a hung renderer. Reopen only with a reproduction in a normal browser.
