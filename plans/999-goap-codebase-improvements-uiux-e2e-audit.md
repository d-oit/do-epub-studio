...
Symptom investigated and attributed (2026-09-20, follow-up round): the reader was seen mounting an empty root after a client-side login navigation, and a later run showed the automation tab becoming unresponsive. Both were observed only inside the managed headless-browser session used for this verification. Re-running with a fresh browser showed the app's own traffic completing normally (`/progress`, `/highlights`, `/comments`, `/bookmarks`, `/feedback`, `/insights/sync`, the signed EPUB fetch — all 200), followed by `workerd` logging a broken pipe from the vanished client, and **no renderer process left alive**: the harness had reaped the tab, which is why screenshot and evaluate calls then hung. Nothing in the app failed, the Playwright reader lanes (`demo-login.spec.ts`, `login-and-book-load.spec.ts`) exercise the same login → reader path in CI, and the earlier blank snapshots carry no captured page error. Recorded as an automation-harness artifact rather than a product defect; if it ever appears in a real browser it needs a fresh reproduction.
