...
| `harness` | Map the harness-engineering feedforward guides and feedback sensors, and run the self-correction protocol when a computational sensor fires. Use when a sensor fails (cargo check, test, clippy, fmt), before making code changes, or when setting up agent context for a new task. Triggers: "harness", "sensor fire", "CI failure", "self-correction", "swarm". |
...
| `pr-roast-batch-close` | Reviews ALL open PRs in a repository at once, roasts each one for codebase impact, provides recommendations, and closes PRs with zero meaningful impact. Use for repository hygiene sweeps and PR backlog cleanup. |
...
_Generated by ./scripts/generate-available-skills.py_
