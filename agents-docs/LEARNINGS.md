...
- **Atomic single-use of a JSON array in SQLite needs a single conditional UPDATE**: the recovery-code read-modify-write was race-prone; replaced with one `UPDATE ... SET col = (SELECT json_group_array(value) FROM json_each(col) WHERE value != ?) WHERE ... AND EXISTS(SELECT 1 FROM json_each(col) WHERE value = ?)`. Note there is no real-DB unit harness in `packages/schema`, so this SQL is verified only by the shape-assertion test + D1 at runtime.
