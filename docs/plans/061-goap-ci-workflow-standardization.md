...
Close all remaining actionable items identified across docs/plans/: standardize CI workflows with run-name, concurrency, timeout-minutes, and FORCE_JAVASCRIPT_ACTIONS_TO_NODE24; add missing coverage thresholds to ui package; sync AGENTS.md coverage thresholds with actual configs; fix broken documentation references.
...
| T9 | eslint.config.js: ~60→~4 remaining; docs/plans/010 → docs/plans/archive/010 | ✅ Done |
