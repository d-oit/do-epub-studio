...
- **vs reader-ui-ux**: Use `reader-ui-ux` for building and maintaining the core `d.o. EPUB Studio` design system and 2026 standards. Use `impeccable` for high-end design critique, advanced motion, typography polish, and specialized UI "crafting" that goes beyond standard components.
- **vs anti-ai-slop**: Use `anti-ai-slop` for overall code quality and structural AI patterns. Use `impeccable` for UI-specific "slop" (e.g., generic AI layout reflexes, gradient text, or "ghost cards").
- **vs accessibility-auditor**: Use `accessibility-auditor` for deep, exhaustive accessibility compliance. `impeccable` includes high-level accessibility checks (contrast, reduced motion) as part of its design flow but is not a replacement for a full accessibility audit.
