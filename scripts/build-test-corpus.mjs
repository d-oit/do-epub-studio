...
function opf(chapters, extras = []) {
  const items = chapters
    .map(
      (ch, i) => `    <item id="ch${i + 1}" href="${ch.href}" media-type="application/xhtml+xml"/>`,
    )
    .join('\n');
  const spine = chapters.map((ch, i) => `    <itemref idref="ch${i + 1}"/>`).join('\n');
  const ex = extras
    .map((e) => `    <item id="${e.id}" href="${e.href}" media-type="${e.type}"/>`)
    .join('\n');
...
function genImageHeavy() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    const paras = Array.from(
      { length: 5 },
      (_, j) =>
        `  <p>Chapter ${i}, paragraph ${j + 1}. This chapter contains images and text content for benchmarking image-heavy EPUB rendering.</p>`,
    ).join('\n');
    chapters.push({
      title: `Chapter ${i}`,
      href: `chapter${i}.xhtml`,
...
function genManyChapters() {
  ...
  for (let i = 1; i <= 1300; i++) {
    chapters.push({
      title: `Chapter ${i}`,
      href: `ch${String(i).padStart(3, '0')}.xhtml`,
...
for (let i = 2; i < args.length; i++) {
  if (args[i] === '--output-dir' && args[i + 1]) outDir = args[++i];
}
...
for (const { name, gen, target } of corpus) {
  ...
  console.log(
    `  ${name.padEnd(25)} ${(data.length / 1024).toFixed(1).padStart(8)} KB  (target: ${target})`,
  );
}
