...
// eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- Map is initialized above with 'library' key
const DEFAULT_PATH = ICON_PATHS.get('library')!;
...
export function NavIcon({ icon, className = '' }: { icon: string; className?: string }) {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <path d={ICON_PATHS.get(icon) ?? DEFAULT_PATH} />
    </svg>
  );
}
