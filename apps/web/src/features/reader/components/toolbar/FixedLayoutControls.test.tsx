...
describe('FixedLayoutControls', () => {
  ...
  it('contains an aria-live region for zoom announcements', () => {
    render(
      <FixedLayoutControls
        isOpen
        onClose={vi.fn()}
  ...
  it('handles non-discrete zoom values correctly for button states', () => {
    render(
      <FixedLayoutControls
        isOpen
        onClose={vi.fn()}
        zoom={1.1} // between 1.0 and 1.25
        spread="auto"
        onSetZoom={vi.fn()}
        onSetSpread={vi.fn()}
        t={t}
  ...
  it('calls onSetZoom with the next discrete step when zooming in from a custom value', async () => {
    const onSetZoom = vi.fn();
    render(
      <FixedLayoutControls
        isOpen
  ...
  it('calls onSetZoom with the previous discrete step when zooming out from a custom value', async () => {
    const onSetZoom = vi.fn();
    render(
      <FixedLayoutControls
        isOpen
