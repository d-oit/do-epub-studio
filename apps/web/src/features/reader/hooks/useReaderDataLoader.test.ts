...
const mockBookmarks: Bookmark[] = [
  { id: 'b1', locator: { cfi: 'cfi1' }, label: 'mark', createdAt: '2024-01-01' },
];
...
describe('useReaderDataLoader', () => {
  ...
  it('does nothing when sessionToken is null', () => {
    ...
    renderHook(() => {
      useReaderDataLoader({ sessionToken: null, bookId: 'book1', ...setters });
    });
  ...
  it('does nothing when bookId is null', () => {
    const setters = createMockSetters();
    renderHook(() => {
      useReaderDataLoader({ sessionToken: 'token', bookId: null, ...setters });
    });
    expect(fetchHighlights).not.toHaveBeenCalled();
    expect(logClientEvent).not.toHaveBeenCalled();
  });
  ...
  it('fetches from server and sets all data on success', async () => {
    ...
    renderHook(() => {
      useReaderDataLoader({ sessionToken: 'token', bookId: 'book1', ...setters });
    });
  ...
  it('falls back to offline progress when server fetch fails', async () => {
    ...
    await waitFor(() => {
      expect(setters.setProgress).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  it('restores offline annotations when server fetch fails', async () => {
    ...
    await waitFor(() => {
      expect(setters.setHighlights).toHaveBeenCalledWith([
        expect.objectContaining({ id: 'h1', cfiRange: 'cfi' }),
      ]);
    });
    expect(setters.setComments).toHaveBeenCalledWith([
      expect.objectContaining({ id: 'c1', body: 'body' }),
    ]);
    expect(setters.setBookmarks).toHaveBeenCalledWith([
      expect.objectContaining({ id: 'b1', label: 'label' }),
    ]);
    expect(logClientEvent).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('logs reader.offline_cache_error at debug level when offline processing throws', async () => {
    ...
    vi.mocked(getAnnotations).mockResolvedValue([]);
    const setters = createMockSetters();
    setters.setProgress.mockImplementation(() => {
