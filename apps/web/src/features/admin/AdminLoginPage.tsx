...
export function AdminLoginPage() {
  ...
  const handlePasskey = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const data = await apiRequest<AdminLoginResponse>('/api/admin/login/mfa/verify', {
        method: 'POST',
  ...
  const handleRecovery = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiRequest<AdminLoginResponse>('/api/admin/login/mfa/recovery-verify', {
        method: 'POST',
        body: JSON.stringify({
          email: recoveryEmail,
        ...
        <section data-testid="admin-login-card" className="glass-card w-full p-5 sm:p-7 lg:p-8">
          ...
          <div className="mt-4 text-center">
            <Button
              ...
              onClick={() => {
                void navigate(
                  // eslint-disable-next-line i18next/no-literal-string -- route path; disable sits on the literal because prettier splits the call across lines
                  '/admin/recover',
                );
              }}
          ...
          <div className="mt-8 pt-6 border-t border-border text-center space-y-3">
            <p className="text-sm text-foreground-muted">{t('admin.login.readerDescription')}</p>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => {
                void navigate(
                  // eslint-disable-next-line i18next/no-literal-string -- route path; see note above
                  '/login',
                );
              }}
