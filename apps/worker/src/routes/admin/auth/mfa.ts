...
import { verifyPasskeyAuthentication } from './login-mfa';
import type { AuthApp, RouteContext } from './types';

// =============================================================================
// Route handlers for admin MFA management + passkey authentication. Each is a
// top-level named function so it stays readable and independently verifiable;
// `registerMfa` below is a thin set of registrations.
// =============================================================================
...
async function mfaStatus(c: RouteContext): Promise<Response> {
...
async function mfaAuthenticateStart(c: RouteContext): Promise<Response> {
  const admin = c.get('adminUser');
  const existing = await listPasskeys(c.env, admin.id);
...
async function mfaAuthenticateVerify(c: RouteContext): Promise<Response> {
  ...
  await updatePasskeyCounter(c.env, result.credential.credential_id, result.newCounter);
...
async function mfaRemovePasskey(c: RouteContext): Promise<Response> {
  const { currentPassword } = c.req.valid('json') as z.infer<typeof MfaRemoveSchema>;
  const admin = c.get('adminUser');
...
async function mfaRecoveryCodesRegenerate(c: RouteContext): Promise<Response> {
  const { currentPassword } = c.req.valid('json') as z.infer<typeof RecoveryCodeRegenSchema>;
  const admin = c.get('adminUser');
