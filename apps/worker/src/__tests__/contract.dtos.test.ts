...
describe('Shared DTO schema round-trips', () => {
  it('AccessRequestSchema — accepts valid, rejects missing email', () => {
    expect(
      AccessRequestSchema.safeParse({ bookSlug: 'b', email: 'r@example.com', password: 'pw' })
        .success,
    ).toBe(true);
  ...
  it('RecoveryRequestSchema — accepts valid, rejects invalid email', () => {
    expect(RecoveryRequestSchema.safeParse({ bookSlug: 'b', email: 'r@example.com' }).success).toBe(
      true,
    );
    expect(RecoveryRequestSchema.safeParse({ bookSlug: 'b', email: 'not-email' }).success).toBe(
      false,
    );
  });
  ...
  it('CreateGrantSchema — accepts builder data, rejects non-uuid bookId', () => {
    ...
    expect(
      CreateGrantSchema.safeParse({ bookId: g.bookId, email: g.email, mode: g.mode }).success,
    ).toBe(true);
    expect(CreateGrantSchema.safeParse({ bookId: 'not-uuid', email: 'a@b.com' }).success).toBe(
      false,
    );
  });
  ...
  it('ProgressUpdateSchema — accepts builder percent, rejects >100', () => {
    ...
    expect(
      ProgressUpdateSchema.safeParse({
        locator: { ...msLocator, cfi },
        progressPercent: p.progressPercent,
      }).success,
    ).toBe(true);
    expect(
      ProgressUpdateSchema.safeParse({ locator: msLocator, progressPercent: 101 }).success,
    ).toBe(false);
  });

  it('BookmarkCreateSchema — accepts valid, rejects locator missing chapterRef', () => {
    const bm = createBookmarkBuilder().withLabel('x').build();
    expect(BookmarkCreateSchema.safeParse({ locator: msLocator, label: bm.label }).success).toBe(
      true,
    );
    expect(
      BookmarkCreateSchema.safeParse({ locator: { cfi: 'epubcfi(/1)', selectedText: 'hi' } })
        .success,
    ).toBe(false);
  });
  ...
  it('HighlightCreateSchema — accepts builder color, rejects non-hex color', () => {
    ...
    expect(HighlightCreateSchema.safeParse({ locator: msLocator, color: hl.color }).success).toBe(
      true,
    );
    expect(HighlightCreateSchema.safeParse({ locator: msLocator, color: 'red' }).success).toBe(
      false,
    );
  });
  ...
  it('CommentCreateSchema — accepts builder body, rejects empty body', () => {
    ...
    expect(CommentCreateSchema.safeParse({ body: c.body, visibility: c.visibility }).success).toBe(
      true,
    );
  ...
  it('TelemetryPayloadSchema — accepts valid batch, rejects bad level', () => {
    expect(
      TelemetryPayloadSchema.safeParse({ logs: [{ level: 'info', traceId: 't', event: 'e' }] })
        .success,
    ).toBe(true);
    expect(
      TelemetryPayloadSchema.safeParse({ logs: [{ level: 'verbose', traceId: 't', event: 'e' }] })
        .success,
    ).toBe(false);
  });
  ...
  it('UploadCompleteSchema — accepts valid upload, rejects empty storageKey', () => {
    expect(
      UploadCompleteSchema.safeParse({ storageKey: 'books/x/f.epub', originalFilename: 'f.epub' })
        .success,
    ).toBe(true);
    expect(
      UploadCompleteSchema.safeParse({ storageKey: '', originalFilename: 'f.epub' }).success,
    ).toBe(false);
  });
});
