...
export function createEmailTransport(env: Env): EmailTransport {
  ...
  logAppError('email.transport.fallback', new Error('EMAIL_SEND binding not configured — emails will be logged but not delivered. Bind an Email Sending integration in the Cloudflare dashboard to enable delivery.'), {
