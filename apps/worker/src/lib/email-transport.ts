...
export function createEmailTransport(env: Env): EmailTransport {
  ...
  logAppInfo('email.transport.fallback', {
    message:
      'EMAIL_SEND binding not configured — emails will be logged but not delivered. Bind an Email Sending integration in the Cloudflare dashboard to enable delivery.',
