...
function makeEnv(count = 3): Env {
  ...
  const statement = { bind: vi.fn(() => ({ run: first, first: vi.fn().mockResolvedValue({ request_count: count }) })) };
  return {
...
describe('D1 rate-limit fallback', () => {
  ...
  it('uses the distributed result when the Durable Object answers', async () => {
    ...
    const result = await checkRateLimit(makeEnv(), 'invite', 'key', { maxRequests: 5, windowMs: 1000 });
  ...
  it('uses D1 when the Durable Object is unavailable', async () => {
    ...
    const result = await checkRateLimit(makeEnv(), 'invite', 'key', { maxRequests: 2, windowMs: 1000 });
